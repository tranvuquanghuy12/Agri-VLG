...
def run_stage1_finetuning(args, logger, model, preprocess, tokenized_text_prompts):
    ...
    # check zeroshot acc
    zs_test_acc = -1
    if args.check_zeroshot or args.method == 'zeroshot' or True: # Always check for summary table
        logger.info(f"Check Zero-shot Acc ......")
        # DISABLE GAT during Zero-shot as it's not trained yet
        original_use_gat = model.use_gat
        model.use_gat = False
        zs_test_acc = run_zeroshot(args, test_loader, model, logger, loss, logit_scale, classifier_head)
        model.use_gat = original_use_gat
    ...
    return zs_test_acc, test_acc, best_model_path, test_loader_copy, wsft_test_acc
...
if __name__ == '__main__':
    ...
    args = parse_args()
    print(f"DEBUG: args.method before set_logger: {args.method}")
    ...
    zs_test_acc, stage1_acc, stage1_best_model_path, test_loader, wsft_test_acc = run_stage1_finetuning(args, logger, model, preprocess, tokenized_text_prompts)
    ...
    result_summary = f'{args.dataset},{stage1_method},{args.data_source},{args.cls_init},{args.shots},{args.seed},{args.retrieval_split},{round(zs_test_acc,1)},{round(stage1_acc,1)},{round(wsft_test_acc,1)},{round(stage2_lp_acc,1)},{round(stage2_fsft_acc,1)}'
    
    table = f"""
{'='*60}
📊 BẢNG TỔNG HỢP KẾT QUẢ (DATASET: {args.dataset.upper()})
{'-'*60}
| Cấu hình thí nghiệm: {args.data_source.upper()} | {args.shots}-SHOT | BATCH: {args.bsz} |
{'-'*60}
| Giai đoạn                  | Accuracy (%) | Ý nghĩa kỹ thuật           |
{'-'*60}
| 0. CLIP Mặc định (Zero-shot) | {round(zs_test_acc, 1):<12} | Baseline của OpenAI CLIP   |
| 1. Stage 1 (SGR Training)    | {round(stage1_acc, 1):<12} | Sau khi học RAL + Few-shot |
| 2. WiSE-FT (Ensemble)        | {round(wsft_test_acc, 1):<12} | Trộn trọng số để giữ gốc   |
| 3. Stage 2 (Linear Probe)    | {round(stage2_lp_acc, 1):<12} | Chỉ tinh chỉnh lớp Head     |
| 4. Stage 2 (FSFT - FINAL)    | {round(stage2_fsft_acc, 1):<12} | KẾT QUẢ CUỐI CÙNG          |
{'='*60}
"""
    logger.info(f'\n{table}')
    logger.info(f'CSV Summary: {result_summary}')