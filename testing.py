...
def validate(args, data_loader, model, logger, loss, logit_scale, classifier_head = None,
    ...
    with torch.no_grad():
        ...
        for i, val_data in enumerate(data_loader):
            ...
            # Tự động phát hiện ảnh thô [B, 3, H, W] hay đặc trưng [B, D]
            is_image = (inputs.ndim == 4)
            ...
            if i == 0:
                # In ra 5 mẫu đầu tiên để debug nhãn
                for idx in range(min(5, labels.size(0))):
                    logger.info(f"DEBUG: Sample {idx} - True: {labels[idx].item()}, Pred: {pred[idx].item()}")
