...
def set_params(args, model, classifier_head, logger, dataset_classifier_head=None):
    ...
    if args.lr_gat is None:
        args.lr_gat = args.lr_classifier

    params_classifier = [{'params': classifier_head.parameters(), 'lr': args.lr_classifier}]
    params_visual = [{'params': list(model.visual.parameters()), 'lr': args.lr_backbone}]
    params_gat = [{'params': list(model.gat.parameters()), 'lr': args.lr_gat}]
    ...
    if args.method == "zeroshot":
        ...
        for param in model.parameters():
            param.requires_grad = False
        params = params_classifier + params_gat
        logit_scale = torch.tensor([4.60517]).to(device=args.device)
        ...
        params = params_classifier + params_visual + params_gat
