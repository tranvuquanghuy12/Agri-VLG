...
def prompt_maker(metrics: dict, dataset_name: str, name_type='most_common_name'):
    prompts = {}
    if dataset_name in ['semi-aves', 'semi_aves']:
    ...
    for i, key in enumerate(metrics.keys()):
        # Robust name extraction: fallback to 'name' if specified type is missing
        if name_type in metrics[key]:
            label = metrics[key][name_type]
        elif 'name' in metrics[key]:
            label = metrics[key]['name']
        else:
            # Last resort: use the key itself or the first available value
            label = metrics[key].get('name', list(metrics[key].values())[0])
